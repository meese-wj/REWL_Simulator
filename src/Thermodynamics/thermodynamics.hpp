...
template<typename energy_t, typename logdos_t, typename obs_t,
         class Obs_enum_t, class Obs_container_t>
obs_t Thermodynamics<energy_t, logdos_t, obs_t, Obs_enum_t, Obs_container_t>::find_maximum_exponent( const energy_t Tvalue, const logdos_t * const logdos_array ) const
{
...
void Thermodynamics<energy_t, logdos_t, obs_t,
                    Obs_enum_t, Obs_container_t>::calculate_thermodynamics( const size_t system_size,
                                                                            const logdos_t * const logdos_array,
                                                                            const Obs_container_t * const observables_obj ) const
{
    obs_t partition = 0.;
    ...
    for ( size_t Tidx = 0; Tidx != num_T; ++Tidx )
    {
        ...
        {
            partition += exp( get_reduced_exponent(bin, Tvalue, max_exponent, logdos_array) );
        }
        ...
        for ( size_t bin = 0; bin != num_energy_bins; ++bin )
        {
            ...
            canonical_observables[ Tidx * total_observables + convert(Energy_Obs::internal_energy) ] += energy_value * exp( get_reduced_exponent(bin, Tvalue, max_exponent, logdos_array) );
            canonical_observables[ Tidx * total_observables + convert(Energy_Obs::internal_energy2) ] += energy_value * energy_value * exp( get_reduced_exponent(bin, Tvalue, max_exponent, logdos_array) );
            canonical_observables[ Tidx * total_observables + convert(Energy_Obs::entropy) ] += logdos_array[bin] * exp( get_reduced_exponent(bin, Tvalue, max_exponent, logdos_array) );
            
            // Compute the extra observables
            for ( size_t ob = 0; ob != convert(Obs_enum_t::NUM_OBS); ++ob  )
            {
                if ( ob != convert(Obs_enum_t::counts_per_bin) )
                    canonical_observables[ Tidx * total_observables + convert(Energy_Obs::NUM_OBS) + ob ] += ( observables_obj -> get_observable( ob, bin ) ) * exp( get_reduced_exponent(bin, Tvalue, max_exponent, logdos_array ) );
                else
                    canonical_observables[ Tidx * total_observables + convert(Energy_Obs::NUM_OBS) + ob ] += ( observables_obj -> get_observable( ob, bin ) );
            }
        }
