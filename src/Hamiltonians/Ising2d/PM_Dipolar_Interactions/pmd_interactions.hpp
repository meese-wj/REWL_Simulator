                            ...
                            const std::uint32_t Lx, const std::uint32_t Ly )
{
    ...
    energy_t const_term = -1. * ( square_magnitude<energy_t, energy_t>( difference ) >= 1. &&  square_magnitude<energy_t, energy_t>( difference ) < sqrt(2.)  );
    ...
    return const_term + (1. - 8. * cos_theta_sq + 8. * cos_theta_sq * cos_theta_sq) / square_magnitude<energy_t, energy_t>( difference );
}
...
energy_t PMNI_binsize_contribution( const energy_t phonon_coupling, const std::uint32_t Lx, const std::uint32_t Ly, const energy_t moment_value=1. )
{
    ...
    energy_t binsize = -2 * moment_value * pmd_ints.calculate_energy_per_spin( 0, ferromagnetic_ground_state_spins[0], ferromagnetic_ground_state_spins );
    delete [] ferromagnetic_ground_state_spins;
